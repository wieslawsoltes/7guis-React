...
public sealed partial class MainPage : Page
{
    ...
    private async Task VerifyInteractionAsync(string sample)
    {
        ...
        var assertion = sample switch
        {
            ...
            "temperature" => "document.querySelectorAll('.temperature-row input')[1]?.value === '212'",
            "flight-booker" => "document.querySelector('.confirmation')?.textContent.startsWith('One-way flight booked')",
            "timer" => "Number(document.querySelector('.timer-readout strong')?.textContent) > 0",
            "crud" => "document.querySelectorAll('.crud-grid input')[0]?.value === 'Grace' && document.querySelectorAll('.crud-grid input')[1]?.value === 'Hopper'",
            "crud-redux" => "document.querySelectorAll('.redux-crud input')[0]?.value === 'Grace' && document.querySelectorAll('.redux-crud input')[1]?.value === 'Hopper'",
